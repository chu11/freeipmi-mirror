/*****************************************************************************\
 *  $Id: ipmipower_check.c,v 1.85 2008-05-19 18:47:18 chu11 Exp $
 *****************************************************************************
				    ...
				    uint32_t buffer_len)
{
  ...
  if (pkt == AUTHENTICATION_CAPABILITIES_V20_RES
	...
	ierr_exit("ipmipower_check_authentication_code(%s:%d): "
       ...
       * circumstances.
       */
      if ((args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_CHECK_UNEXPECTED_AUTHCODE)
	  && !rv
	  && check_authcode_retry_flag)
	{
...
ipmipower_check_outbound_sequence_number(ipmipower_powercmd_t ip, packet_type_t pkt)
{
   ...
   */
  if (args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_BIG_ENDIAN_SEQUENCE_NUMBER)
    {
  ...
  if (args.common.driver_type == IPMI_DEVICE_LAN_2_0 && !seq_num)
    goto out;
...
ipmipower_check_session_id(ipmipower_powercmd_t ip, packet_type_t pkt) 
{
  ...
  if ((args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_ACCEPT_SESSION_ID_ZERO)
      && !session_id)
    return (1);
...
ipmipower_check_rakp_2_key_exchange_authentication_code(ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  if (args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_INTEL_2_0_SESSION)
	...
	strcpy(username_buf, args.common.username);
  ...
  if (args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_SUPERMICRO_2_0_SESSION)
    {
   ...
   * have the problem.
   */
  if (args.common.workaround_flags & IPMI_WORKAROUND_FLAGS_SUN_2_0_SESSION
...
ipmipower_check_payload_pad(ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert(ip);
  assert(PACKET_TYPE_VALID_RES(pkt));
  assert(args.common.driver_type == IPMI_DEVICE_LAN_2_0
         && (pkt == SET_SESSION_PRIVILEGE_LEVEL_RES
             || pkt == GET_CHASSIS_STATUS_RES
             || pkt == CHASSIS_CONTROL_RES
             || pkt == CLOSE_SESSION_RES));
