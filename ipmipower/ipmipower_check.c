...
ipmipower_check_checksum (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert (ip);
  assert (PACKET_TYPE_IPMI_1_5_SETUP_RESPONSE (pkt)
	  || PACKET_TYPE_IPMI_SESSION_PACKET_RESPONSE (pkt)); /* 1.5 or 2.0 */
                                     ...
                                     unsigned int buflen)
{
  ...
  assert (ip);
  assert (pkt == ACTIVATE_SESSION_RESPONSE
	  || PACKET_TYPE_IPMI_SESSION_PACKET_RESPONSE (pkt));
  assert (buf && buflen);

  /* IPMI 1.5 Checks */
  if (pkt == ACTIVATE_SESSION_RESPONSE
      || cmd_args.common.driver_type == IPMI_DEVICE_LAN)
    {
      ...
      else /* PACKET_TYPE_IPMI_SESSION_PACKET_RESPONSE (pkt) */
        {
...
ipmipower_check_outbound_sequence_number (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert (ip);
  assert (PACKET_TYPE_IPMI_SESSION_PACKET_RESPONSE (pkt));
...
ipmipower_check_session_id (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert (ip);
  assert (PACKET_TYPE_IPMI_2_0_SETUP_RESPONSE (pkt)
	  || PACKET_TYPE_IPMI_SESSION_PACKET_RESPONSE (pkt));
  ...
  else /* PACKET_TYPE_IPMI_2_0_SETUP_RESPONSE (pkt) */
    {
      fiid_obj_t obj_cmd;
...
ipmipower_check_network_function (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  if (pkt == GET_CHASSIS_STATUS_RESPONSE
  ...
  else /* pkt == AUTHENTICATION_CAPABILITIES_RESPONSE
...
ipmipower_check_rakp_2_key_exchange_authentication_code (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert (ip);
  assert (pkt == RAKP_MESSAGE_2_RESPONSE);
...
ipmipower_check_rakp_4_integrity_check_value (ipmipower_powercmd_t ip, packet_type_t pkt)
{
  ...
  assert (ip);
  assert (pkt == RAKP_MESSAGE_4_RESPONSE);
