		  ...
		  struct common_cmd_args *cmd_args)
{
  switch (key)
	...
	{
	  cmd_args->auth_type = IPMI_AUTH_TYPE_NONE;
	}
      else 
	if (strcasecmp (arg, "md2") == 0)
	  {
	    cmd_args->auth_type = IPMI_AUTH_TYPE_MD2;
	  }
	else 
	  if (strcasecmp (arg, "md5") == 0)
	    {
	      cmd_args->auth_type = IPMI_AUTH_TYPE_MD5;
	    }
	  else 
	    if (strcasecmp (arg, "plain") == 0)
	      {
		cmd_args->auth_type = IPMI_AUTH_TYPE_STRAIGHT_PASSWD_KEY;
	      }
	    else 
	      if (strcasecmp (arg, "oem") == 0)
		{
		  cmd_args->auth_type = IPMI_AUTH_TYPE_OEM_PROP;
		}
	      else 
		{
		  argp_usage (state);
	...
	{
	  cmd_args->priv_level = IPMI_PRIVILEGE_LEVEL_CALLBACK;
	}
      else 
	if (strcasecmp (arg, "user") == 0)
	  {
	    cmd_args->priv_level = IPMI_PRIVILEGE_LEVEL_USER;
	  }
	else 
	  if (strcasecmp (arg, "operator") == 0)
	    {
	      cmd_args->priv_level = IPMI_PRIVILEGE_LEVEL_OPERATOR;
	    }
	  else 
	    if (strcasecmp (arg, "admin") == 0)
	      {
		cmd_args->priv_level = IPMI_PRIVILEGE_LEVEL_ADMIN;
	      }
	    else 
	      if (strcasecmp (arg, "oem") == 0)
		{
		  cmd_args->priv_level = IPMI_PRIVILEGE_LEVEL_OEM;
		}
	      else 
		{
		  argp_usage (state);
