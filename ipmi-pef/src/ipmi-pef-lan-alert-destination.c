...
#define IPMI_PEF_MAXIPADDRLEN 16
#define IPMI_PEF_MAXMACADDRLEN 24

static pef_err_t
destination_type_get (ipmi_pef_state_data_t *state_data,
...
static pef_err_t
destination_type_set (ipmi_pef_state_data_t *state_data,
                      uint8_t destination_selector,
                      uint8_t alert_destination_type,
...
static pef_err_t
alert_destination_type_checkout (ipmi_pef_state_data_t *state_data,
...
static pef_err_t
alert_destination_type_commit (ipmi_pef_state_data_t *state_data,
                               const struct section *sect,
...
static pef_diff_t
alert_destination_type_diff (ipmi_pef_state_data_t *state_data,
                             const struct section *sect,
                             const struct keyvalue *kv)
{
  uint8_t get_val;
...
static pef_err_t
alert_acknowledge_checkout (ipmi_pef_state_data_t *state_data,
                            const struct section *sect,
                            struct keyvalue *kv)
{
  uint8_t alert_acknowledge;
...
static pef_err_t
alert_acknowledge_commit (ipmi_pef_state_data_t *state_data,
                          const struct section *sect,
                          const struct keyvalue *kv)
{
  uint8_t destination_selector;
...
static pef_diff_t
alert_acknowledge_diff (ipmi_pef_state_data_t *state_data,
                        const struct section *sect,
                        const struct keyvalue *kv)
{
  uint8_t get_val;
...
static pef_err_t
alert_acknowledge_timeout_checkout (ipmi_pef_state_data_t *state_data,
                                    const struct section *sect,
                                    struct keyvalue *kv)
{
  uint8_t alert_acknowledge_timeout;
...
static pef_diff_t
alert_acknowledge_timeout_diff (ipmi_pef_state_data_t *state_data,
                                const struct section *sect,
                                const struct keyvalue *kv)
{
  uint8_t get_val;
...
static pef_err_t
alert_retries_checkout (ipmi_pef_state_data_t *state_data,
                        const struct section *sect,
                        struct keyvalue *kv)
{
  uint8_t alert_retries;
...
static pef_err_t
alert_retries_commit (ipmi_pef_state_data_t *state_data,
                      const struct section *sect,
                      const struct keyvalue *kv)
{
  uint8_t destination_selector;
...
static pef_diff_t
alert_retries_diff (ipmi_pef_state_data_t *state_data,
                    const struct section *sect,
                    const struct keyvalue *kv)
{
  uint8_t get_val;
...
pef_validate_t
alert_retries_validate (ipmi_pef_state_data_t *state_data,
                        const struct section *sect,
                        const char *value)
{
  long int conv;
...
static pef_err_t
destination_addresses_get (ipmi_pef_state_data_t *state_data,
                           uint8_t destination_selector,
                           ...
                           unsigned int alert_mac_address_len)
{
  ...
  char tmp_ip[IPMI_PEF_MAXIPADDRLEN + 1];
  char tmp_mac[IPMI_PEF_MAXMACADDRLEN+1];
  pef_err_t ret;

  memset(tmp_ip, '\0', IPMI_PEF_MAXIPADDRLEN + 1);
  memset(tmp_mac, '\0', IPMI_PEF_MAXMACADDRLEN+1);

  if ((ret = get_bmc_lan_conf_destination_addresses (state_data,
                                                     destination_selector,
                                                     &tmp_alert_gateway,
                                                     tmp_ip,
                                                     IPMI_PEF_MAXIPADDRLEN + 1,
                                                     tmp_mac,
                                                     IPMI_PEF_MAXMACADDRLEN+1)) != PEF_ERR_SUCCESS)
    return ret;
  ...
  if (alert_ip_address)
    { 
      if (alert_ip_address_len >= IPMI_PEF_MAXIPADDRLEN + 1)
        memcpy(alert_ip_address, tmp_ip, IPMI_PEF_MAXIPADDRLEN + 1);
      else
        return PEF_ERR_FATAL_ERROR;
    }
  if (alert_mac_address)
    { 
      if (alert_mac_address_len >= IPMI_PEF_MAXMACADDRLEN + 1)
        memcpy(alert_mac_address, tmp_mac, IPMI_PEF_MAXMACADDRLEN + 1);
      else
        return PEF_ERR_FATAL_ERROR;
    }
...
static pef_err_t
destination_addresses_set (ipmi_pef_state_data_t *state_data,
                           uint8_t destination_selector,
                           uint8_t alert_gateway,
...
static pef_err_t
alert_gateway_checkout (ipmi_pef_state_data_t *state_data,
                        const struct section *sect,
                        struct keyvalue *kv)
{
  uint8_t gateway;
...
static pef_err_t
alert_gateway_commit (ipmi_pef_state_data_t *state_data,
                      const struct section *sect,
                      const struct keyvalue *kv)
{
  uint8_t destination_selector;
...
static pef_diff_t
alert_gateway_diff (ipmi_pef_state_data_t *state_data,
                    const struct section *sect,
                    const struct keyvalue *kv)
{
  uint8_t get_val;
...
static pef_err_t
alert_ip_address_checkout (ipmi_pef_state_data_t *state_data,
                           const struct section *sect,
                           struct keyvalue *kv)
{
  pef_err_t ret;
...
static pef_err_t
alert_ip_address_commit (ipmi_pef_state_data_t *state_data,
                         const struct section *sect,
                         const struct keyvalue *kv)
{
  uint8_t destination_selector;
...
static pef_diff_t
alert_ip_address_diff (ipmi_pef_state_data_t *state_data,
                       const struct section *sect,
                       const struct keyvalue *kv)
{
  char alert_ip[IPMI_PEF_MAXIPADDRLEN + 1];
