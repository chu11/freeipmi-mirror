                         ...
                         uint32_t most_recent_erase_timestamp)
{
  ...
  if (n != 4)
    {
      SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_SYSTEM_ERROR);
      return (-1);
    }
      ...
      for (i = 0; i < *record_ids_count; i++)
        {
          if (record_ids[i] == record_id)
            {
              SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_CACHE_CREATE_DUPLICATE_RECORD_ID);
              return (-1);
            }
        }
                       ...
                       void *create_callback_data)
{
  ...
  if ((fd = open (filename, open_flags, 0644)) < 0)
    {
      if (!(cache_create_flags & IPMI_SDR_CACHE_CREATE_FLAGS_OVERWRITE)
          && errno == EEXIST)
        SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_CACHE_CREATE_CACHE_EXISTS);
      else if (errno == EPERM
               || errno == EACCES
               || errno == EISDIR
               || errno == EROFS)
        SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_PERMISSION);
      else if (errno == ENAMETOOLONG
               || errno == ENOENT
               || errno == ELOOP)
        SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_FILENAME_INVALID);
      else if (errno == ENOSPC
               || errno == EMFILE
               || errno == ENFILE)
        SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_FILESYSTEM);
      else
        SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_SYSTEM_ERROR);
      goto cleanup;
    }
  ...
  if (!record_count)
    {
      SDR_CACHE_SET_ERRNUM (ctx, IPMI_SDR_ERR_CACHE_CREATE_INVALID_RECORD_COUNT);
      goto cleanup;
    }
