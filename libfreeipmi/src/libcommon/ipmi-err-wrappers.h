...
#define __SENSOR_READ_ERRNO_TO_ERRNUM                                   \
do {                                                                    \
  if (errno == 0)                                                       \
    ...
    ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_SYSTEM_ERROR;                \
} while (0)

#define SENSOR_READ_ERR(expr)                                           \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        __SENSOR_READ_ERRNO_TO_ERRNUM;                                  \
        __SENSOR_READ_TRACE;                                            \
        return (-1);                                                    \
      }                                                                 \
  } while (0)

#define SENSOR_READ_ERR_CLEANUP(expr)                                   \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        __SENSOR_READ_ERRNO_TO_ERRNUM;                                  \
...
#define SENSOR_READ_ERR_OUT_OF_MEMORY(expr)                             \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_OUT_OF_MEMORY;           \
...
#define SENSOR_READ_ERR_OUT_OF_MEMORY_CLEANUP(expr)                     \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_OUT_OF_MEMORY;           \
...
#define SENSOR_READ_ERR_IPMI_ERROR(expr)                                \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_IPMI_ERROR;              \
        __SENSOR_READ_TRACE;                                            \
        return (-1);                                                    \
      }                                                                 \
  } while (0)

#define SENSOR_READ_ERR_IPMI_ERROR_CLEANUP(expr)                        \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_IPMI_ERROR;              \
...
#define SENSOR_READ_ERR_SYSTEM_ERROR(expr)                              \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_SYSTEM_ERROR;            \
...
#define SENSOR_READ_ERR_SYSTEM_ERROR_CLEANUP(expr)                      \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_SYSTEM_ERROR;            \
...
#define SENSOR_READ_ERR_INTERNAL_ERROR(expr)                            \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_INTERNAL_ERROR;          \
...
#define SENSOR_READ_ERR_INTERNAL_ERROR_CLEANUP(expr)                    \
  do {                                                                  \
    if (!(expr))                                                        \
      {                                                                 \
        ctx->errnum = IPMI_SENSOR_READ_CTX_ERR_INTERNAL_ERROR;          \
