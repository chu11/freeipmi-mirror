		       ...
		       fiid_obj_t obj_cmd_rq)
{
  ...
  
  {
    ...
    pkt_len = _ipmi_lan_pkt_rq_size2 (dev, obj_cmd_rq);
    pkt = alloca (pkt_len);
    ERR (pkt);
    memset (pkt, 0, pkt_len);
...
ipmi_lan_check_chksum (uint8_t *pkt, uint64_t pkt_len)
{
  ...
  if ((msg_hdr_len1 = fiid_template_block_len_bytes(tmpl_hdr_session,
						    (uint8_t *)"auth_type",
						    (uint8_t *)"session_id")) < 0)
    return (-1);
  ...
  if ((msg_hdr_len2 = fiid_template_field_len_bytes(tmpl_hdr_session,
						    (uint8_t *)"ipmi_msg_len")) < 0)
    return (-1);
