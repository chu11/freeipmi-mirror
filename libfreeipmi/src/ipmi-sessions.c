...
fiid_template_t tmpl_lan_session_header =
  {
...
fill_hdr_session  (uint8_t auth_type, uint32_t inbound_seq_num, uint32_t session_id, uint8_t *auth_code_data, uint32_t auth_code_data_len, fiid_obj_t obj_hdr)
{
  ...
  if ((rv = fiid_obj_template_compare(obj_hdr, tmpl_lan_session_header)) < 0)
    return (-1);
...
check_hdr_session_authcode (uint8_t *pkt, uint64_t pkt_len, uint8_t auth_type, uint8_t *auth_code_data, uint32_t auth_code_data_len)
{
  ...
  ERR(!((auth_type_index = fiid_template_field_start_bytes (tmpl_lan_session_header, (uint8_t *)"auth_type")) < 0));
  ...
  ERR(!((auth_code_index = fiid_template_field_start_bytes (tmpl_lan_session_header, (uint8_t *)"auth_code")) < 0));
  ...
  if (auth_type == IPMI_SESSION_AUTH_TYPE_MD2
      || auth_type == IPMI_SESSION_AUTH_TYPE_MD5)
    {
      ...
      ERR(!((session_id_index = fiid_template_field_start_bytes (tmpl_lan_session_header, (uint8_t *)"session_id")) < 0));
      ERR(!((session_seq_num_index = fiid_template_field_start_bytes (tmpl_lan_session_header, (uint8_t *)"session_seq_num")) < 0));
      ERR(!((session_id_len = fiid_template_field_len_bytes (tmpl_lan_session_header, (uint8_t *)"session_id")) < 0));
      ERR(!((session_seq_num_len = fiid_template_field_len_bytes (tmpl_lan_session_header, (uint8_t *)"session_seq_num")) < 0));
      ERR(!((data_index = fiid_template_len_bytes (tmpl_lan_session_header)) < 0));
