...
int8_t
check_hdr_session_authcode (uint8_t *pkt, uint64_t pkt_len, uint8_t auth_type, uint8_t *auth_code, uint32_t auth_code_len)
{
  ...
  if (!pkt 
      || (auth_code && auth_code_len > IPMI_SESSION_MAX_AUTH_CODE_LEN))
    {
      errno = EINVAL;
      return (-1);
    }
  ...
  if (auth_code)
    memcpy(auth_code_buf, auth_code, auth_code_len);
...
int8_t 
check_hdr_session_authcode_calc (uint8_t *pkt, uint64_t pkt_len, uint8_t auth_type, uint8_t *auth_code_data, uint32_t auth_code_data_len)
{
  uint8_t auth_type_recv;
  int32_t rmcp_hdr_len, auth_type_index, auth_code_index;
  uint32_t auth_type_offset, auth_code_offset;
