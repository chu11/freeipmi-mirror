                           ...
                           fiid_obj_t obj_hdr)
{
  ...
  if (payload_type == IPMI_PAYLOAD_TYPE_OEM_EXPLICIT)
    {
      FIID_OBJ_SET (obj_hdr, "oem_iana", oem_iana);
      FIID_OBJ_SET (obj_hdr, "oem_payload_id", oem_payload_id);
    }
  FIID_OBJ_SET (obj_hdr, "session_id", session_id);
  FIID_OBJ_SET (obj_hdr, "session_sequence_number", session_sequence_number);
                      ...
                      fiid_obj_t obj_cmd)
{
  ...
  FIID_OBJ_TEMPLATE_COMPARE(obj_cmd, tmpl_rmcpplus_payload);
