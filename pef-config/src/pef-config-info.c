...
pef_info (pef_config_state_data_t *state_data)
{
  ...
  if (!(obj_cmd_rs = Fiid_obj_create (state_data->pstate,
                                      tmpl_cmd_get_pef_capabilities_rs)))
    goto cleanup;
  ...
  if (Fiid_obj_get (state_data->pstate,
                    obj_cmd_rs, 
                    "action_support.diagnostic_interrupt", 
                    &val) < 0)
    goto cleanup;
  pstdout_printf (state_data->pstate,
  ...
  if (Fiid_obj_get (state_data->pstate,
                    obj_cmd_rs,
                    "oem_event_record_filtering_supported",
                    &val) < 0)
    goto cleanup;
  pstdout_printf (state_data->pstate,
