...
pef_config_config_sections_create (pef_config_state_data_t *state_data)
{
  ...
  if (!(s = pef_config_pef_conf_section_get (state_data)))
    goto cleanup;
  if (config_section_append (&sections, s) < 0)
    goto cleanup;

  if (!(s = pef_config_community_string_section_get (state_data)))
    goto cleanup;
  if (config_section_append (&sections, s) < 0)
    goto cleanup;
  ...
  for (i = 0; i < number_of_lan_alert_destinations; i++)
    {
      if (!(s = pef_config_lan_alert_destination_section_get(state_data, i+1)))
	goto cleanup;
      if (config_section_append (&sections, s) < 0)
	goto cleanup;
    }
  ...
  for (i = 0; i < number_of_alert_strings; i++)
    {
      if (!(s = pef_config_alert_string_section_get(state_data, i+1)))
	goto cleanup;
      if (config_section_append (&sections, s) < 0)
	goto cleanup;
    }
  ...
  for (i = 0; i < number_of_alert_policy_entries; i++)
    {
      if (!(s = pef_config_alert_policy_table_section_get(state_data, i+1)))
	goto cleanup;
      if (config_section_append (&sections, s) < 0)
	goto cleanup;
    }
  ...
  for (i = 0; i < number_of_event_filters; i++)
    {
      if (!(s = pef_config_event_filter_table_section_get(state_data, i+1)))
	goto cleanup;
      if (config_section_append (&sections, s) < 0)
	goto cleanup;
    }
